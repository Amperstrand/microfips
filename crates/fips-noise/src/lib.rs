...
mod responder_tests {
    ...
    fn test_xk_initiator_responder_full_handshake() {
        ...
        let mut msg1 = [0u8; XK_HANDSHAKE_MSG1_SIZE];
        ...
        let mut msg2 = [0u8; XK_HANDSHAKE_MSG2_SIZE];
        let msg2_len = responder
            .write_message2(&SECRET_B, &epoch2, &mut msg2)
            .unwrap();
        assert_eq!(msg2_len, XK_HANDSHAKE_MSG2_SIZE);
        assert_eq!(initiator.read_message2(&msg2).unwrap(), epoch2);

        let mut msg3 = [0u8; XK_HANDSHAKE_MSG3_SIZE];
        let msg3_len = initiator

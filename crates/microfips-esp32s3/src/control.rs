...
const GPIO_FUNC_IN_SEL_BASE: usize = 0x6000_9000;
const U0RXD_SIG_IN_IDX: usize = 44;
const GPIO44_NUM: u32 = 44;
...
fn uart0_init_rx() {
    unsafe {
        let gpio44_in_sel = (GPIO_FUNC_IN_SEL_BASE + 4 * U0RXD_SIG_IN_IDX) as *mut u32;
        write_volatile(gpio44_in_sel, GPIO44_NUM | (1 << 7));
    }
}
...
fn handle_version() {
    esp_println::println!("microfips-esp32s3 {}", env!("CARGO_PKG_VERSION"));
}
...
fn handle_reset() {
    ...
    unsafe {
        core::ptr::write_volatile(RTC_CNTL_BASE as *mut u32, 1 << 31);
    }
    loop {
        core::hint::spin_loop();
    }

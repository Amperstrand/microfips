...
| Noise XX | Forward-compat link handshake (FIPS next wire) | fips-noise 53/53 + full protocol suite green under `--features std,noise-xx` (CI-enforced); **CI regression gate since 2026-09-03 (#195)**: xx-interop workflow builds upstream jmcorgan/next at pinned `NEXT_REF` (f1ff410f) and proves `fips-handshake --features noise-xx` + a 60s `microfips-sim` heartbeat session against it on loopback, paths-filtered so unrelated merges never wait on upstream; **live interop 2026-09-02 (#179): sim ↔ next-branch daemon** — FMP v1 negotiation agreed (version 1, peer profile full), connection promoted, sustained heartbeats both directions; **bench hardware 2026-09-02 (#193): S3 ↔ next daemon end-to-end** (pinned mDNS discovery, FMP v1 agreed, one sustained 90s+ session, daemon-initiated rekey followed over XX; regression-guarded by fips-lab `test_bench_xx`). The bench session also found+fixed a silent-peer policy bug (direct-path sends now feed the policy); **MMP report interop fixed 2026-09-03 (#196)** — slim next-wire report layouts + negotiated send gating (Leaf: ReceiverReports only, data-gated), daemon parses every report and measures RTT/loss/jitter (sim rtt=1.1ms, bench rtt≈99-119ms over WiFi) |
...
| fips-lab | Scenario infrastructure + regression assertions | 95 tests collect; 9 live scenarios (rekey soak fast/stock, link death, mdns pinned, rekey self-init, mesh full-session, rekey bidirectional, L2CAP bring-up, rekey soak long, bench XX); cross-project Bench Testing Playbook in its docs/ (11 patterns + 2026-09-02 amendments); D0WD bench tier (build_d0wd_l2cap + ftdi_tap + boards.toml atoms); XX tier (BenchXxDaemon + noise-xx build features + FIPS_NEXT_BIN gate) |
...
| Noise XX live interop | **CI-gated since 2026-09-03 (#195)** — xx-interop workflow vs pinned upstream-next daemon on every XX-path change; **sim↔daemon verified 2026-09-02 (#179)** against FIPS next (0.6.0-dev); **bench hardware verified 2026-09-02 (#193)** — S3 ran XX/FMP-v1 end-to-end vs a next-branch bench daemon (G·22, `test_bench_xx` scenario); **MMP reports verified 2026-09-03 (#196)** — daemon-side RTT/loss/jitter live, zero Malformed lines (two greens with the #196 asserts) | #195, #196 |

...
async fn main(_spawner: Spawner) {
    ...
    let mut builder = Builder::new(driver, usb_cfg, &mut cfg_desc, &mut bos_desc, &mut [], &mut ctl_buf);

    let mut class = CdcAcmClass::new(&mut builder, &mut cdc_st, CDC_PKT as u16);
    let mut usb = builder.build();

    let usb_fut = usb.run();
    let fips_fut = fips_task(&mut class, &mut led, &my_pub, rng);
...
async fn fips_task<'d, T: embassy_stm32::usb::Instance + 'd>(
    ...
    my_pub: &[u8; 33],
    rng: &'static mut Rng<'static, peripherals::RNG>,
) {
    ...
    loop {
        ...
        match handshake(class, my_pub, rng, &mut rbuf, &mut rpos, &mut rlen).await {
            Ok((ks, kr, them, _us)) => {
                info!("handshake ok");
...
async fn handshake<'d, T: embassy_stm32::usb::Instance + 'd>(
    ...
    let (mut st, ep) =
        noise::NoiseIkInitiator::new(&eph, &MCU_SECRET, &VPS_PUB).map_err(|_| Err::Invalid)?;

    let epoch: [u8; 8] = [0x01, 0, 0, 0, 0, 0, 0, 0];

    let mut n1 = [0u8; 256];
    let n1len = st.write_message1(my_pub, &epoch, &mut n1).map_err(|_| Err::Invalid)?;

    let mut f1 = [0u8; 256];
    let f1len = fmp::build_msg1(0, &n1[..n1len], &mut f1);

    cdc_send_frame(class, &f1[..f1len]).await?;
...
fn handle(kr: &[u8; 32], data: &[u8]) -> Result<(), Err> {
    ...
    match m {
        fmp::FmpMessage::Established {
            ...
            let dl = noise::aead_decrypt(kr, counter, hdr, encrypted, &mut dec).map_err(|_| Err::Decrypt)?;
